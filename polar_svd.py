...
def _sum_sq(x):
    e, s = _gamma(2 * x.numel() + 1, torch.float64), float(x.double().square().sum(dtype=torch.float64))
...
def _prod_norm_hi(a, b, symmetric=False):
    ...
    if not isfinite(vmax := float((w := a @ (b @ v) if symmetric else b.mT @ (a.mT @ (a @ (b @ v)))).max())) or vmax <= 0.0:
        return inf
    v = (w / vmax).clamp_min(torch.finfo(b.dtype).tiny)
    a, b, v = a.double(), b.double(), v.double()
    q = float((((a @ (b @ v)) if symmetric else b.mT @ (a.mT @ (a @ (b @ v)))) / v).max())
...
def _residual_plan(n, dtype, c_hi):
    blk, panels = min(n, 64), (n + min(n, 64) - 1) // min(n, 64)
    gb, best = _gamma(blk, dtype), (inf, 1)
    for group in range(1, panels + 1):
        gg, gh = _gamma(group, dtype), _gamma((panels + group - 1) // group, dtype)
        err = na(fsum((gb * c_hi, gg * (1.0 + gb) * c_hi, gh * (1.0 + (1.0 + gb) * (1.0 + gg) * c_hi), n * n * torch.finfo(dtype).tiny)), inf)
...
def _triangular_inverse_lower(r, norm_err, sigma_hi, base_lo, sigma_ub):
    try:
        ...
        return 0.0
    ysq = _sum_sq(y)
    inv_bound = min(ysq[1], _prod_norm_hi(y.mT, y, True))
    inv_hi = na(sqrt(inv_bound), inf)
...
def spectral_envelope(a, triangular=False):
    ...
    x = (a / scale).to(a.dtype).contiguous()
    tlo, thi = _sum_sq(x)
    ...
    if triangular and x.dtype == torch.float32 and x.shape[0] == x.shape[1] and bool((diag := x.diagonal().abs()).min() > 0.0):
        sigma_ub = max(0.0, na(float(diag.min()) - norm_err, 0.0))
        if _qdwh_work(max(sigma_lo, sigma_ub), sigma_hi, x.shape[0], x.dtype) + x.shape[0] ** 3 < _qdwh_work(sigma_lo, sigma_hi, x.shape[0], x.dtype):
            sigma_lo = max(sigma_lo, _triangular_inverse_lower(x, norm_err, sigma_hi, sigma_lo, sigma_ub))
...
def _auto_reduce(m, n, dtype):
    shifts = [shift for _, shift, _, _ in _qdwh_schedule(0.0, dtype)]
    nwork = sum(4.0 * n**3 + n * n + (4.0 if shift <= _gamma(n, dtype) else 1.0) * n**3 / 3.0 for shift in shifts)
    mwork = sum(4.0 * m * n * n + m * n + (4.0 if shift <= _gamma(m, dtype) else 1.0) * n**3 / 3.0 for shift in shifts)
    return 19.0 * n**3 / 3.0 + nwork < mwork
...
def qdwh_polar(a, *, triangular=False):
    ...
    for beta, shift, sqrt_shift, eta in steps:
        ...
        q, _ = torch.linalg.qr(qr_work, mode="reduced")
        x = beta * x + (eta / sqrt_shift) * (q[:m] @ q[m:].mT)
        routes["qr"] += 1
    core = x.mT @ x0
...
def _finish(w, x0, core, info):
    ...
    s = s_unit.to(torch.float64) * float(info["alpha"]) if x0.dtype == torch.float32 else s_unit * float(info["alpha"])
    s = s if x0.dtype == torch.float32 and bool((s > float(torch.finfo(x0.dtype).max)).any()) else s.to(x0.dtype)
    u = w @ vc[:, :rank] if trusted else (x0 @ vh[:rank].mT) / s_unit[:rank].reshape(1, -1)
    if rank and not trusted:
        u, r = torch.linalg.qr(u, mode="reduced")
        u = u * torch.where(r.diagonal() < 0.0, -1.0, 1.0).to(u.dtype).reshape(1, -1)
    if rank < vh.shape[0]:
        q, _ = torch.linalg.qr(torch.cat((u, torch.eye(x0.shape[0], vh.shape[0], dtype=x0.dtype, device=x0.device)), dim=1), mode="reduced")
...
def svd(a, *, reduce_tall="auto"):
    ...
    if a.shape[0] > a.shape[1] and (reduce_tall is True or (reduce_tall == "auto" and _auto_reduce(*a.shape, a.dtype))):
        ...
        work = torch.cat((ur, hh.new_zeros((hh.shape[0] - hh.shape[1], ur.shape[1]))))
        ...
        return SVDResult(torch.ormqr(hh, tau, work, left=True, transpose=False), s, vh, info)
